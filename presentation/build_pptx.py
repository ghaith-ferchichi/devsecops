...
OUT = "/opt/devsecops/BTE_Security_AI_Agent_Soutenance.pptx"

# ---------- Palette : Secure Intelligence (Light) ----------
WHITE   = RGBColor(0xFF, 0xFF, 0xFF)
MIST    = RGBColor(0xF2, 0xF5, 0xF8)
NAVY    = RGBColor(0x0B, 0x25, 0x45)
SLATE_T = RGBColor(0x1C, 0x2B, 0x3A)
CYAN_D  = RGBColor(0x0E, 0x8F, 0xA3)
CYAN    = RGBColor(0x22, 0xD3, 0xEE)
INDIGO  = RGBColor(0x6D, 0x28, 0xD9)
VIOLET  = RGBColor(0x7C, 0x5C, 0xFC)
GOLD    = RGBColor(0xE0, 0xA1, 0x00)
...
def header(s, kicker, title, num=None):
    """Standard content-slide header: kicker + title + cyan rule + footer."""
    rect(s, 0, 0, 13.333, 7.5, fill=WHITE)            # canvas
    rect(s, 0, 0, 0.16, 7.5, fill=NAVY)               # left spine
    if kicker:
        txt(s, 0.62, 0.46, 11.0, 0.35, [[P(kicker.upper(), 12, CYAN_D, True, HEAD)]])
    txt(s, 0.62, 0.74, 12.1, 0.9, [[P(title, 27, NAVY, True, HEAD)]])
    rect(s, 0.64, 1.55, 1.5, 0.05, fill=CYAN)         # accent rule
    footer(s, num)
...
def footer(s, num=None):
    ...
    # AI-signature ribbon (cyan->violet) bottom-right
    rib = rect(s, 10.9, 7.16, 1.7, 0.12, fill=CYAN, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
    set_gradient(rib, CYAN, VIOLET, 0)
    if num is not None:
        txt(s, 12.05, 7.10, 0.6, 0.3, [[P(f"{num:02d}", 10, NAVY, True, HEAD)]],
...
rect(s, 0, 0, 13.333, 0.22, fill=CYAN); set_gradient(s.shapes[-1], CYAN, VIOLET, 0)
...
try:
    ...
    pass
txt(s, 0.9, 2.35, 11.5, 0.4, [[P("RAPPORT DE STAGE DE FIN D'ÉTUDES  ·  MASTÈRE PROFESSIONNEL SSI", 13, CYAN_D, True, HEAD)]])
txt(s, 0.9, 2.85, 11.6, 1.6, [
    [P("BTE Security AI Agent", 44, NAVY, True, HEAD)],
    [P("Agent IA agentique pour la revue automatisée de sécurité du code", 21, SLATE_T, False, BODY)],
])
rect(s, 0.94, 4.55, 2.2, 0.06, fill=CYAN); set_gradient(s.shapes[-1], CYAN, VIOLET, 0)
...
rib = rect(s, 0, 7.28, 13.333, 0.22, fill=CYAN); set_gradient(rib, VIOLET, CYAN, 0)
...
rect(s, 0, 0, 13.333, 0.18, fill=CYAN); set_gradient(s.shapes[-1], CYAN, VIOLET, 0)
txt(s, 1.0, 1.5, 11.3, 0.5, [[P("LE PROBLÈME", 14, CYAN, True, HEAD)]])
txt(s, 1.0, 2.2, 11.3, 3.0, [
    [P("Une seule injection SQL fusionnée dans une API de paiement", 30, WHITE, True, HEAD)],
    [P("peut compromettre toute la banque.", 30, CYAN, True, HEAD)],
])
txt(s, 1.0, 4.6, 11.3, 1.2, [
    [P("Aujourd'hui à la BTE, la revue de sécurité est ", 18, RGBColor(0xC9,0xD6,0xE5), False, BODY),
     P("manuelle", 18, GOLD, True, HEAD),
     P(", prend ", 18, RGBColor(0xC9,0xD6,0xE5), False, BODY),
     P("jusqu’à 24 h", 18, GOLD, True, HEAD),
     P(",", 18, RGBColor(0xC9,0xD6,0xE5), False, BODY)],
    [P("et dépend entièrement du relecteur de garde.", 18, RGBColor(0xC9,0xD6,0xE5), False, BODY)],
])
footer_dark = rect(s, 0, 7.3, 13.333, 0.2, fill=VIOLET); set_gradient(footer_dark, VIOLET, CYAN, 0)
...
rect(s, 0.8, 2.0, 0.12, 2.1, fill=CYAN, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
set_gradient(s.shapes[-1], CYAN, VIOLET, 90)
...
rect(s, 0, 0, 13.333, 7.5, fill=NAVY)
rect(s, 0, 0, 13.333, 0.18, fill=CYAN); set_gradient(s.shapes[-1], CYAN, VIOLET, 0)
txt(s, 1.0, 1.4, 11.3, 0.5, [[P("CONCLUSION", 14, CYAN, True, HEAD)]])
txt(s, 1.0, 2.1, 11.3, 2.2, [
    [P("Un agent IA agentique, déployé en production à la BTE,", 26, WHITE, True, HEAD)],
    [P("qui sécurise chaque Pull Request ", 26, WHITE, True, HEAD),
     P("sans qu'aucun code ne quitte la banque.", 26, CYAN, True, HEAD)],
])
for i,(big,lab) in enumerate([("24 h → 15 min","délai"),("5/5","détection"),("100 % local","conformité BCT")]):
    x = 1.0 + i*3.9
    rect(s, x, 4.4, 3.6, 1.3, fill=RGBColor(0x12,0x31,0x57), line=None, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
    txt(s, x, 4.6, 3.6, 0.6, [[P(big, 22, CYAN, True, HEAD)]], align=PP_ALIGN.CENTER)
    txt(s, x, 5.25, 3.6, 0.4, [[P(lab, 12, RGBColor(0xC9,0xD6,0xE5), False, BODY)]], align=PP_ALIGN.CENTER)
txt(s, 1.0, 6.1, 11.3, 0.6, [[P("Plateforme remise à la BTE  ·  Merci de votre attention", 16, GOLD, True, HEAD)]])
rib = rect(s, 0, 7.3, 13.333, 0.2, fill=VIOLET); set_gradient(rib, VIOLET, CYAN, 0)
