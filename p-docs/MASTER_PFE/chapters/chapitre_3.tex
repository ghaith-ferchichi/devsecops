...
\section{Présentation de l'architecture logicielle}
    ...
    Cette section décrit l'architecture de déploiement de la plateforme. Elle présente
    d'abord son organisation en couches fonctionnelles, puis l'inventaire détaillé des
    conteneurs et enfin les règles de routage qui régissent son point d'entrée unique.
    ...
    \subsection{Inventaire des conteneurs}
        ...
        \hline
        \texttt{postgres} & \texttt{postgres:\allowbreak{}16-alpine} & interne & \textasciitilde33 Mo & BD + checkpoints \\
        \hline
        \texttt{redis} & \texttt{redis:\allowbreak{}7-alpine} & interne & \textasciitilde5 Mo & Cache + dédup \\
        \hline
        \texttt{nginx} & \texttt{nginx:\allowbreak{}alpine} & 80, 443 & \textasciitilde3 Mo & Reverse proxy \\
        \hline
        ...
        \hline
        \texttt{victoriametrics} & \texttt{victoria-\allowbreak{}metrics} & interne & \textasciitilde97 Mo & Stockage 90j \\
        \hline
        \texttt{node-exporter} & \texttt{prom/\allowbreak{}node-exporter} & \texttt{host:9100} & \textasciitilde10 Mo & Métriques host \\
        \hline
        \texttt{open-webui} & \texttt{open-\allowbreak{}webui:\allowbreak{}main} & 3001 & \textasciitilde300 Mo & UI Ollama \\
        \hline
        \end{tabular}
...
\section{Description de la conception du logiciel}
    ...
    La conception de l'agent se décompose en sous-systèmes faiblement couplés, chacun
    responsable d'une fonction précise. Les sous-sections qui suivent détaillent tour à
    tour l'inférence LLM, la revue de \textit{Pull Request}, l'analyse du diff,
    l'assistant de chat, la persistance des données et l'observabilité.

    \subsection{Sous-système d'inférence LLM}

        Le sous-système d'inférence fournit les modèles de langage exploités par le reste
        de la plateforme, du pipeline de revue jusqu'à l'assistant de chat. Sa logique
        d'ensemble est décrite ci-après, suivie de la configuration retenue pour chaque
        fonction de fabrique de modèle.
        ...
        Cette durée traduit une contrainte matérielle assumée : l'analyse de 15 à
        25 minutes par \textit{Pull Request} découle directement
    ...
    \subsection{Sous-système de revue de Pull Request}
        ...
        Le sous-système de revue de \textit{Pull Request} orchestre l'enchaînement complet
        des étapes d'analyse, de la réception de l'événement GitHub jusqu'à la publication
        du verdict. Sa logique repose sur un graphe d'états présenté ci-après, avant les
        diagrammes d'interaction qui détaillent la réception du webhook puis le déroulé du
        pipeline.
        ...
        \begin{figure}[H]
            \centering
            \frame{\includegraphics[width=\columnwidth,height=0.88\textheight,keepaspectratio]{langgraph_state_graph}}
        ...
        \begin{figure}[H]
            \centering
            \frame{\includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{inter_webhook}}
        ...
        \begin{figure}[H]
            \centering
            \frame{\includegraphics[width=\textwidth,height=0.88\textheight,keepaspectratio]{inter_pipeline}}
    ...
    \subsection{Sous-système d'analyse du diff}
        ...
        Le sous-système d'analyse du diff prépare les données transmises au modèle et
        fiabilise les commentaires publiés sur la \textit{Pull Request}. Sa logique interne
        est décrite ci-après, suivie du format de diff annoté fourni au modèle pour qu'il
        cible précisément les lignes concernées.
        ...
        \begin{figure}[H]
            \centering
            \frame{\includegraphics[width=\columnwidth,height=0.7\textheight,keepaspectratio]{diff_parser_state_machine}}
    ...
    \subsection{Sous-système d'assistant de chat opérationnel}
        ...
        Le sous-système d'assistant de chat donne à l'équipe sécurité un accès en langage
        naturel à l'état de la plateforme et à la base de connaissances. Son architecture,
        fondée sur une boucle ReAct, est présentée ci-après puis illustrée par un diagramme
        d'interaction.
        ...
        \begin{figure}[H]
            \centering
            \frame{\includegraphics[width=\columnwidth,height=0.88\textheight,keepaspectratio]{react_loop_architecture}}
        ...
        \begin{figure}[H]
            ...
            \caption{Système anti-hallucination (conception personnelle)}
        ...
        invention probabiliste de valeurs ; \texttt{num\_ctx}, porté à 6 144 tokens dans
        la configuration propre au chat (contre 4 096 pour la fabrique de
        classification du pipeline), laisse
        ...
        \begin{figure}[H]
            \centering
            \frame{\includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{inter_react}}
    ...
    \subsection{Sous-système de persistance}
        ...
        Le sous-système de persistance conserve l'ensemble des données produites par la
        plateforme, des revues archivées jusqu'à l'état interne du pipeline. Son
        organisation logique est présentée ci-après, avant la description détaillée des
        ...
        \begin{figure}[H]
            \centering
            \frame{\includegraphics[width=\textwidth]{database_er_diagram}}
    ...
    \subsection{Sous-système d'observabilité}
        ...
        \begin{figure}[H]
            \centering
            \frame{\includegraphics[width=\textwidth]{monitoring_stack}}
