...
(242 Go), un AlertManager silencieusement cassé depuis le déploiement, VictoriaMetrics
resté en panne neuf jours, ou les trois bugs cumulatifs du parser de revue PR (regex
intolérant aux espaces, \texttt{json.loads} strict, modèle omettant le JSON). Chacun de
ces incidents est devenu la carte prioritaire du sprint suivant et a donné lieu à un
correctif déployé en production avant la clôture de l'itération.
...
injection d'instructions dans le code soumis. Sur le terrain de
l'inférence, l'évaluation conduite en post-Sprint 8 d'un second moteur (LocalAI, servant
le même Qwen2.5-Coder 7B que la production) a tranché en faveur d'Ollama, plus rapide de
22\,\% sur un matériel identique : 5,49 contre 4,50 tokens par seconde. Le sprint 8.1 a
poussé plus loin cette comparaison en instrumentant les deux backends de chat ; une
télémétrie A/B y mesure la latence du premier \textit{token}, la durée totale et le débit
en sortie pour chaque couple \texttt{backend}~$\times$~\texttt{model}. Ce dispositif a
d'ailleurs révélé un bug latent du \textit{template} \texttt{ChatML-with-im\_\allowbreak{}sep}
du modèle Phi-4, qui renvoyait silencieusement zéro \textit{token} et serait passé inaperçu
sans cette instrumentation. La sandbox LocalAI ouvre par ailleurs l'accès à des modèles
absents de la bibliothèque Ollama, tel Qwen3-Coder MoE 30B/3B-actifs ; un \textit{fine-tuning}
sur des CVE bancaires reste une piste à explorer.
