...
\section{DevOps, CI/CD et DevSecOps}
    ...
    \subsection{Du DevOps au DevSecOps}
        ...
        trois principes. Le \textit{Security} consiste à exprimer
    ...
    \subsection{L'OWASP Top 10}
        ...
        catégories au moyen d'une référence explicite, par exemple A01:2025 --- Broken
        Access Control ou A05:2025 --- Injection. Le tableau \ref{tab:owasp2025} en présente
        la liste complète.
        ...
        \hline
        \end{tabular}
        \caption{Les dix catégories de l'OWASP Top 10:2025}
        \label{tab:owasp2025}
        \end{table}
...
\section{Stack applicative et observabilité}
    ...
    \subsection{Base de données relationnelle}
        ...
        Dans notre architecture, une même base de données relationnelle porte deux
        usages bien séparés. Elle est d'abord une \textbf{base de connaissances sécurité}
        qui conserve les revues de \textit{Pull Requests}, les résultats de scanners et
        les profils de risque par dépôt, et c'est elle qui alimente les requêtes de
        tendances. Elle sert par ailleurs à la \textbf{persistance des checkpoints} de
        l'orchestrateur, ce qui permet à un pipeline de 15 à 25 minutes de survivre au
        redémarrage de son conteneur.

    \subsection{Cache et déduplication}

        Un cache en mémoire intervient à plusieurs endroits sensibles du pipeline. Il
        assure la \textbf{déduplication des webhooks}, pour éviter qu'un même événement
        soit traité deux fois lorsque GitHub réémet une notification après \textit{timeout}. Il
        applique aussi le \textbf{rate limiting} qui plafonne le nombre de pipelines
        simultanés par dépôt et protège ainsi le moteur d'inférence de la surcharge. Il
        sert enfin de \textbf{cache des résultats de scanners} : sur un nouveau \textit{push} vers
        la même branche, les résultats sont renvoyés en quelques millisecondes au lieu de
        relancer une analyse complète.

    \subsection{Observabilité}

        Un système qui exécute des pipelines de plusieurs minutes, dépend d'un moteur
        d'inférence local et orchestre cinq scanners externes ne reste exploitable que
        s'il est instrumenté. La stack d'observabilité commence par la \textbf{collecte de
        métriques} (durée du pipeline, latence des appels LLM, taux d'erreur des scanners,
        occupation disque), que le \textbf{stockage long terme} conserve sur 90 jours pour
        l'analyse de tendances et que des dashboards rendent lisibles par la
        \textbf{visualisation}. À cela s'ajoute un mécanisme d'\textbf{alerting}, qui
        signale les anomalies (saturation disque, indisponibilité du moteur LLM, échecs
        répétés de scanners) avant qu'elles ne touchent les utilisateurs.
