...
\section{Implémentation par sprints Scrumban}
    ...
    \subsection{Sprint 1 — Fondations : VPS, Docker et webhook GitHub}
        ...
        Docker Docker et à valider la réception des webhooks GitHub.
        ...
        courante d'administration système Linux. La suite de
        ...
        La configuration de performance d'Ollama Ollama est critique pour un fonctionnement CPU-only.
        ...
        présente la configuration Ollama dans le fichier \texttt{docker-compose.\allowbreak{}yml}.
    ...
    \subsection{Sprint 2 — Pipeline de sécurité : scanners et LLM}
        ...
        \texttt{GhaiethFerchichi/\allowbreak{}Vuln-application}.
    ...
    \subsection{Sprint 3 — Revue combinée et commentaires inline GitHub}
        ...
        \begin{figure}[H]
            ...
            \caption{PR~\#18 — revue de sécurité, partie~1}
        ...
        \begin{figure}[H]
            ...
            \caption{PR~\#18 — revue de sécurité, partie~2}
        ...
        \begin{figure}[H]
            ...
            \caption{PR~\#18 — revue de sécurité, partie~3}
    ...
    \subsection{Sprint 6 — Monitoring host et optimisation du chat}
        ...
        le tableau \ref{tab:benchmark_result}. Les modèles \texttt{qwen2.\allowbreak{}5-coder:7b} et
    ...
    \subsection{Sprint 7 — Audit VPS et durcissement}
        ...
        \begin{figure}[H]
            ...
            \caption{Comportement du chat agent}
...
if json_match:
    ...
    \subsection{Post-Sprint 8 — Évaluation d'un second backend d'inférence}
        ...
        La sandbox est définie dans un fichier \texttt{docker-compose.\allowbreak{}localai.\allowbreak{}yml}
        ...
        un volume Docker nommé \texttt{localai\_\allowbreak{}models}, ce qui les protège des
        ...
        \texttt{POST /\allowbreak{}models/\allowbreak{}apply}. Les GGUF HuggingFace hors galerie, comme le
        \texttt{qwen2.\allowbreak{}5-coder-7b} que nous avons choisi pour répliquer le modèle
        ...
        \texttt{/\allowbreak{}build/\allowbreak{}models/} et ignore les sous-dossiers comme \texttt{config/}.
...
mmap: true
        ...
        bifurque vers la sandbox. La fonction \texttt{\_\allowbreak{}parse\_\allowbreak{}backend()} sépare le
...
if backend == "ollama":
    ...
    llm = ChatOllama(base_url=settings.ollama_base_url, model=raw_model,
                     temperature=0.0, num_ctx=_num_ctx,
    ...
    llm = ChatOpenAI(base_url=f"{settings.localai_base_url}/v1",
        ...
        La route \texttt{/\allowbreak{}chat/\allowbreak{}models} fusionne les deux catalogues dans un seul
        ...
        avoir sélectionné le modèle \texttt{localai/\allowbreak{}phi-4} et envoyé une simple
        ...
        \begin{lstlisting}[basicstyle=\footnotesize\ttfamily, breaklines=true, breakatwhitespace=false, frame=none, numbers=none, xleftmargin=1.5em]
No streaming chunk received for 120.0s (model=phi-4, chunks_received=1).
The connection may be alive at the TCP layer but is not producing
content. Tune or disable via the `stream_chunk_timeout` constructor
kwarg or the LANGCHAIN_OPENAI_STREAM_CHUNK_TIMEOUT_S env var.
\end{lstlisting}

        L'analyse a révélé que le client \texttt{ChatOpenAI} de la bibliothèque
        \texttt{langchain\_\allowbreak{}openai} embarque un chien de garde par chunk de cent-vingt
        ...
        \texttt{stream\_\allowbreak{}chunk\_\allowbreak{}timeout=None} au constructeur \texttt{ChatOpenAI} :
        ...
        petite requête synchrone \texttt{POST /\allowbreak{}v1/\allowbreak{}chat/\allowbreak{}completions} avec
        \texttt{max\_\allowbreak{}tokens=4} et un message \og hi \fg{} suffit à forcer LocalAI à
...
async def _prime_localai_model(model: str) -> None:
    """Force LocalAI a charger `model` avant que LangChain
    ne commence le streaming."""
    ...
    try:
        ...
        \hline
        \texttt{agent/\allowbreak{}app/\allowbreak{}routers/\allowbreak{}chat.\allowbreak{}py} & Fonction \texttt{\_\allowbreak{}parse\_\allowbreak{}backend()}, \texttt{\_\allowbreak{}prime\_\allowbreak{}localai\_\allowbreak{}model()}, \texttt{stream\_\allowbreak{}chunk\_\allowbreak{}timeout=None}, deux événements SSE supplémentaires, table \texttt{LOCALAI\_\allowbreak{}MODEL\_\allowbreak{}META} \\
        \hline
        \texttt{agent/\allowbreak{}app/\allowbreak{}config.\allowbreak{}py} & Nouveau champ \texttt{localai\_\allowbreak{}base\_\allowbreak{}url} \\
        \hline
        \texttt{docker-compose.\allowbreak{}localai.\allowbreak{}yml} & Nouveau Compose autonome pour la sandbox LocalAI \\
        \hline
        \texttt{localai/\allowbreak{}config/\allowbreak{}qwen2.\allowbreak{}5-coder-7b.\allowbreak{}yaml} & Nouveau — pointe sur le même GGUF HuggingFace qu'Ollama \\
        \hline
        \texttt{scripts/\allowbreak{}benchmark-backends.\allowbreak{}sh} & Nouveau — comparaison head-to-head des deux backends \\
        \hline
        \end{tabular}
    ...
    \subsection{Sprint 8.1 — Expansion de l'observabilité et correction du \textit{template} Phi-4}
        ...
        la base \texttt{/\allowbreak{}var/\allowbreak{}lib/\allowbreak{}docker/\allowbreak{}image/\allowbreak{}overlayfs/\allowbreak{}layerdb/\allowbreak{}mounts/} dont cAdvisor
        a besoin pour résoudre l'identifiant de couche par conteneur. Tous les
        \textit{cgroups} \texttt{/\allowbreak{}system.\allowbreak{}slice/\allowbreak{}docker-<hash>.\allowbreak{}scope} étaient découverts
        ...
        \texttt{--containerd=/\allowbreak{}run/\allowbreak{}containerd/\allowbreak{}containerd.\allowbreak{}sock} et
        \texttt{--raw\_\allowbreak{}cgroup\_\allowbreak{}prefix\_\allowbreak{}whitelist=/\allowbreak{}system.\allowbreak{}slice/\allowbreak{}docker-} n'a pas permis
        ...
        \texttt{/\allowbreak{}var/\allowbreak{}run/\allowbreak{}docker.\allowbreak{}sock} pour les besoins de ses outils d'introspection
...
async def _poll_docker_stats():
    ...
    while True:
        ...
        Request, qui n'emprunte pas \texttt{\_\allowbreak{}stream\_\allowbreak{}react()}, reste totalement
        inchangé.
...
async def _stream_react(req: ChatRequest):
    ...
    try:
        ...
        chat_requests_total.labels(
            backend=backend, model=raw_model, status=_status).inc()
        if _token_count:
        ...
        instrumentation : le dictionnaire \texttt{LOCALAI\_\allowbreak{}MODEL\_\allowbreak{}META}, qui décrit la
        ...
        \texttt{/\allowbreak{}chat/\allowbreak{}models}. Il a été déplacé à la portée du module pour que le
        \textit{poller} LocalAI puisse l'importer et alimenter la jauge
        \texttt{localai\_\allowbreak{}model\_\allowbreak{}size\_\allowbreak{}gb} à partir de la même source de vérité,
        ...
        alertmanager, node-exporter), les jauges \texttt{ollama\_\allowbreak{}reachable} et
        \texttt{localai\_\allowbreak{}reachable} émises par leurs \textit{pollers} respectifs, et
        la jauge \texttt{container\_\allowbreak{}running\{name=.\allowbreak{}.\allowbreak{}.\allowbreak{}\}} du nouveau \textit{poller}
...
template:
  ...
  completion: |
    {{.Input}}
        ...
        dans \texttt{docker-compose.\allowbreak{}localai.\allowbreak{}yml} qui projette le fichier de l'hôte
        ...
        complet de \texttt{docker compose down/\allowbreak{}up} sans recourir à une intervention
        manuelle via \texttt{docker exec}.

        \textbf{Validation expérimentale.} Un appel direct à l'endpoint
        \texttt{/\allowbreak{}v1/\allowbreak{}chat/\allowbreak{}completions} de LocalAI, en contournant l'agent, retourne
        désormais le contenu \texttt{Pong} avec \texttt{completion\_\allowbreak{}tokens = 3},
        ...
        \texttt{chat\_\allowbreak{}tokens\_\allowbreak{}streamed\_\allowbreak{}total\{model="phi-4"\}} est passé de 0 à 51
...
\section{Résultats}
    ...
    \subsection{Résultat de la revue sur une Pull Request de démonstration}
        ...
        \texttt{GhaiethFerchichi/\allowbreak{}Vuln-application}, ouverte le 28~mai~2026 pour
