...
\section{Implémentation par sprints Scrumban}
    ...
    \subsection{Sprint 1 — Fondations : VPS, Docker et webhook GitHub}
        ...
        \texttt{app/routers/webhooks.py}. L'endpoint
        \texttt{POST /webhooks/github} répond en 202 et confie le reste à une
        ...
        \begin{figure}[H]
            ...
            \caption{Handler FastAPI de réception des webhooks GitHub ---
                     validation HMAC-SHA256, filtrage des événements
                     \textit{pull\_request} et dispatch asynchrone vers le
                     moteur de workflow.}
        ...
        \texttt{application/x-www-form-urlencoded}. Cette seconde branche a
        ...
        Seuls les événements \texttt{pull\_request} dont l'action vaut
        ...
        tâche est généré et la fonction \texttt{dispatch\_event} est
    ...
    \subsection{Sprint 2 — Pipeline de sécurité : scanners et LLM}
        ...
        \texttt{GhaiethFerchichi/Vuln-application}.

        \begin{figure}[H]
            \centering
            \frame{\includegraphics[width=0.88\columnwidth]{scanners_parallel_output}}
            \caption{PR~\#18 — exécution parallèle des cinq scanners, résultats et mise
                     en cache Redis}
    ...
    \subsection{Sprint 3 — Revue combinée et commentaires inline GitHub}
        ...
        des deux appels 14B séquentiels en un seul appel combiné. Le gain porte sur la
        durée totale (scanners, classification 7B et publication GitHub comprises), et
        non sur la seule phase d'inférence du modèle 14B. Le tableau
        \ref{tab:combined_perf} résume cet impact.
        ...
        \texttt{banking\_demo\_final.php} y regroupe cinq failles types :
        injection SQL, secrets en dur, path traversal, injection de commande
        et hachage faible. Les figures~\ref{fig:github_review_sec0}
        à~\ref{fig:slack_notification_pr_result} retracent la revue de bout
        en bout, depuis l'onglet \textit{Conversation} de GitHub jusqu'à la
        notification Slack.

        \begin{figure}[H]
            \centering
            \includegraphics[width=0.92\columnwidth]{pr_comment-security-review_0}
            \caption{PR~\#18 — revue de sécurité, partie~1 : résumé et score
                     de risque (\texttt{HIGH}, verdict \texttt{BLOCK}).}
        ...
        \begin{figure}[H]
            ...
            \caption{PR~\#18 — revue de sécurité, partie~2 : détail des
                     vulnérabilités et résultats SAST.}
        ...
        \begin{figure}[H]
            ...
            \caption{PR~\#18 — revue de sécurité, partie~3 : recommandations,
                     échec du check \texttt{security-ai-agent} et statut du commit.}
        ...
        \begin{figure}[H]
            ...
            \caption{PR~\#18 — revue de qualité de code publiée dans
                     l'onglet \textit{Conversation}.}
        ...
        \begin{figure}[H]
            ...
            \caption{PR~\#18 — suggestion \textit{inline} sur l'injection SQL
                     (\texttt{banking\_demo\_final.php}, bouton \textit{Apply}).}
        ...
        \begin{figure}[H]
            ...
            \caption{PR~\#18 — journaux agent lors de la finalisation
                     (\texttt{pr=18}, cinq \textit{inline}, statut commit
                     \texttt{error}).}
    ...
    \subsection{Sprint 4 — Opérations autonomes et urgence disque}
        ...
        interrompu du modèle \texttt{qwen2.5-coder:32b} avait laissé un blob de
        ...
        \texttt{chat:write.public}, comme le montre la
        figure~\ref{fig:slack_bot_config}. Son \textit{bot token} est injecté
        via \texttt{SLACK\_BOT\_TOKEN}, l'identifiant du canal via
        \texttt{SLACK\_CHANNEL\_ID}.

        \begin{figure}[H]
            \centering
            \frame{\includegraphics[width=0.88\columnwidth]{slack_bot_config}}
            \caption{Configuration OAuth du \textit{bot} Slack — portées
                     \texttt{channels:read}, \texttt{chat:write} et
                     \texttt{chat:write.public}.}
    ...
    \subsection{Sprint 5 — Stack de monitoring Prometheus et Grafana}
        ...
        \texttt{--web.route-prefix}, ce qui déplace le chemin \texttt{/metrics}
        ...
        Après ajout des \texttt{metrics\_path} explicites (\texttt{/prometheus/metrics}
        ...
        \begin{figure}[H]
            ...
            \caption{Dashboard Grafana DevSecOps AI Agent — version Sprint~5
                     (33 panneaux, cinq lignes)}
...
async def _stream_react(req: ChatRequest):
    ...
    try:
        ...
        Entre le Sprint~5 (trente-trois panneaux, cf. figure~\ref{fig:grafana_agent})
        et ce micro-sprint, le dashboard avait déjà été enrichi jusqu'à quarante-cinq
        panneaux. Le Sprint~8.1 y ajoute cinq \textit{rows} supplémentaires : le total
        passe de quarante-cinq à soixante-six \textit{panels}, sans aucune
...
\section{Résultats}
    ...
    \subsection{Résultat de la revue sur une Pull Request de démonstration}
        ...
        \texttt{GhaiethFerchichi/Vuln-application}, ouverte le 28~mai~2026 pour
        un module bancaire de démonstration contenant cinq vulnérabilités types.
        La figure~\ref{fig:pr_full_result} en donne la vue d'ensemble : commentaires
        de revue, suggestions \textit{inline}, verdict \texttt{BLOCK} et échec
        du check \texttt{security-ai-agent}.

        \begin{figure}[H]
            \centering
            \includegraphics[width=0.92\columnwidth]{pr_review_full_result}
            \caption{PR~\#18 — vue d'ensemble de la revue publiée sur GitHub
                     (\texttt{HIGH}, \texttt{BLOCK}, cinq suggestions \textit{inline}).}
        ...
        \hline
        \end{tabular}
        \caption{Résultat de la revue PR de démonstration — dépôt GhaiethFerchichi/Vuln-application}
    ...
    \subsection{Validation de la plateforme}
        ...
        \hline
        Base de données PR reviews & SELECT pr\_reviews — 1 enregistrement (PR~\#18) & \checkmark \\
        \hline
        \end{tabular}
...
du chapitre a présenté le résultat de la validation sur la PR de démonstration~\#18,
avec une revue complète produite en environ quinze minutes, des commentaires
\textit{inline} et des suggestions de correction publiés directement sur GitHub ; la
fourchette de quinze à vingt-cinq minutes reste la cible retenue pour des PR plus volumineuses.
